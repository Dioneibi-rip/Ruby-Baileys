...
const corporateBrowserProfiles = [
    () => Browsers.ubuntu("Chrome"),
    () => Browsers.windows("Chrome"),
    () => Browsers.appropriate("Chrome")
]

const selectCorporateBrowser = () => {
    const index = Math.floor(Math.random() * corporateBrowserProfiles.length)
    return corporateBrowserProfiles[index]()
}
...
const makeWASocketWithTelemetry = async(config = {}) => makeWASocket(await prepareWASocketConfig(config))

...
const extractGroupMetadata = (result) => {
    ...
    const metadata = {
        ...
        participants: getBinaryNodeChildren(group, 'participant').map(({ attrs }) => {
            const pn = attrs.phone_number || (isPnUser(attrs.jid) ? attrs.jid : undefined)
            const lid = attrs.lid || (isLidUser(attrs.jid) ? attrs.jid : undefined)
            const jid = jidNormalizedUser(pn || attrs.jid || lid)

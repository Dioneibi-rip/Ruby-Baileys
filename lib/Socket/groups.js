...
const normalizeParticipantRole = (role) => {
    ...
    if (['creator', 'superadmin', 'owner'].includes(role)) {
        return 'superadmin'
    }
...
const normalizeParticipantJid = (value, fallbackServer) => {
    if (!value) {
        return undefined
    }
    ...
    if (/^\d+$/.test(value)) {
        return jidEncode(value, fallbackServer)
    }
...
const extractGroupMetadata = (result) => {
    ...
    if (descChild) {
        ...
        descOwner = mode === WAMessageAddressingMode.LID ? jidNormalizedUser(descChild.attrs.participant_pn || descChild.attrs.participant) : jidNormalizedUser(descChild.attrs.participant)
    ...
    const metadata = {
        ...
        ownerAlt: group.attrs.creator ? jidNormalizedUser(group.attrs?.creator_pn ? group.attrs.creator_pn : group.attrs.creator) : undefined,
        ...
        participants: getBinaryNodeChildren(group, 'participant').map(({ attrs }) => {
            const id = mode === WAMessageAddressingMode.LID
                ? normalizeParticipantJid(attrs.phone_number || attrs.jid_pn || attrs.jid, 's.whatsapp.net')
                : normalizeParticipantJid(attrs.jid || attrs.phone_number, 's.whatsapp.net')
            const lid = mode === WAMessageAddressingMode.LID
                ? normalizeParticipantJid(attrs.jid || attrs.lid, 'lid')
                : normalizeParticipantJid(attrs.lid, 'lid')

            return {
                id,
                jid: id,
                lid,
                admin: normalizeParticipantRole(attrs.type)
            }
        }),

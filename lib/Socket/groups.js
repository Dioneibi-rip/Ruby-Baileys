...
const makeGroupsSocket = (config) => {
    ...
    const groupFetchThrottleMs = typeof config.groupFetchAllParticipatingThrottleMs === 'number'
        ? Math.max(0, config.groupFetchAllParticipatingThrottleMs)
        : 350
    let groupFetchAllParticipatingMutex = Promise.resolve()
    const { 
        authState, 
        ev, 
        query, 
    ...
    const groupFetchAllParticipatingUnsafe = async () => {
        ...
        if (groupsChild) {
            ...
            for (let i = 0; i < groups.length; i++) {
                ...
                data[meta.id] = meta
                if (groupFetchThrottleMs > 0 && i < groups.length - 1) {
                    await delay(groupFetchThrottleMs)
                }
            }
        }
    ...
    const groupFetchAllParticipating = async () => {
        const task = groupFetchAllParticipatingMutex.then(groupFetchAllParticipatingUnsafe, groupFetchAllParticipatingUnsafe)
        groupFetchAllParticipatingMutex = task.catch(() => undefined)
        return task
    }
...
const extractGroupMetadata = (result) => {
    ...
    const mode = group?.attrs?.addressing_mode === 'lid' ? 'lid' : 'pn'
    ...
    if (descChild) {
        ...
        descOwnerAlt = descChild.attrs.participant ? jidNormalizedUser(descChild.attrs.participant) : undefined
    }
    ...
    const metadata = {
        ...
        ownerAlt: group.attrs.creator_pn ? jidNormalizedUser(group.attrs.creator_pn) : undefined, 

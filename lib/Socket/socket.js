...
const detectIfBusinessAccount = async (queryFn, jid) => {
    try {
        ...
        const list = result?.content?.[0]?.content || []
        return list.some(n => n.tag === 'business')
    } catch {
        return false
    }
}

const getCompanionPlatformId = (isBusiness) => {
    // Business NO acepta WEB
    return isBusiness ? 'android' : 'web'
}
...
const makeSocket = (config) => {
    ...
    // 🔎 Detectar automáticamente si es Business
    const isBusiness = await detectIfBusinessAccount(query, jid)

    logger.info(
        `Pairing por código → ${isBusiness ? 'WhatsApp Business' : 'WhatsApp normal'}`
    )
        ...
        content: [
            {
                ...
                attrs: {
                    ...
                    ...(isBusiness ? { biz: 'true', account_type: 'business' } : {})
                },
                content: [
                    ...(isBusiness ? [{ tag: 'biz', attrs: {} }] : []),
                    ...
                    },
                    {
                        ...
                        attrs: {},
                        content: getCompanionPlatformId(isBusiness)
                    },
                    {

...
const isHostedLidUser = (jid) => typeof jid === 'string' && (jid.includes('@hosted.lid') || jid.endsWith('@lid'))
const isHostedPnUser = (jid) => typeof jid === 'string' && (jid.includes('@hosted.s.whatsapp.net') || jid.endsWith('@s.whatsapp.net'))
...
const makeMessagesSocket = (config) => {
    ...
    const assertSessions = async (jids, force) => {
        ...
        const checkIsPnUser = typeof isPnUser === 'function'
            ? isPnUser
            : (jid) => typeof jid === 'string' && jid.endsWith('@s.whatsapp.net')
        const checkIsLidUser = typeof isLidUser === 'function'
            ? isLidUser
            : (jid) => typeof jid === 'string' && jid.endsWith('@lid')

        const uniqueJids = [...new Set(Array.isArray(jids) ? jids : [])]
        const jidsRequiringFetch = []

        logger.debug({ jids: uniqueJids }, 'assertSessions call with jids')
        ...
        for (const jid of uniqueJids) {
            ...
            const signalId = typeof signalRepository?.jidToSignalProtocolAddress === 'function'
            ...
            if (cachedSession !== undefined) {
                ...
                if (typeof signalRepository?.validateSession === 'function') {
                    const sessionValidation = await signalRepository.validateSession(jid).catch(() => undefined)
                    hasSession = !!sessionValidation?.exists
                } else {
                    const [user, device] = String(signalId).split(':')
                    if (user && typeof authState?.keys?.get === 'function') {
                        const sessionKey = `${user}.${device || 0}`
                        const sessionStore = await authState.keys.get('session', [sessionKey]).catch(() => undefined)
                        hasSession = !!sessionStore?.[sessionKey]
                    }
                }
        ...
        if (jidsRequiringFetch.length) {
            let mappedLids = []
            const pnCandidates = jidsRequiringFetch.filter(jid => !!checkIsPnUser(jid) || !!isHostedPnUser(jid))

            if (signalRepository?.lidMapping && typeof signalRepository.lidMapping.getLIDsForPNs === 'function') {
                const mapped = await signalRepository.lidMapping.getLIDsForPNs(pnCandidates).catch(() => [])
                mappedLids = (Array.isArray(mapped) ? mapped : []).map(a => a?.lid).filter(Boolean)
            } else {
                logger.debug('lidMapping.getLIDsForPNs unavailable, skipping PN->LID expansion')
            }

            const wireJids = [
                ...jidsRequiringFetch.filter(jid => !!checkIsLidUser(jid) || !!isHostedLidUser(jid)),
                ...mappedLids
            ]
            ...
            const result = await query({
                ...
                logger.warn({ error }, 'session fetch query failed, skipping assertSessions fetch')
                return undefined
            })

            if (result && typeof parseAndInjectE2ESessions === 'function') {
                await parseAndInjectE2ESessions(result, signalRepository).catch((error) => {
                    logger.warn({ error }, 'parseAndInjectE2ESessions failed')
                })
                didFetchNewSession = true
            }

            for (const wireJid of wireJids) {
                const sid = typeof signalRepository?.jidToSignalProtocolAddress === 'function'
                    ? signalRepository.jidToSignalProtocolAddress(wireJid)
                    : wireJid
                peerSessionsCache.set(sid, true)
            }
        }
                    ...
                    (async () => {
                        ...
                        if (groupData && Array.isArray(groupData?.participants)) {
                            ...
                            try {
                                ...
                                logger.warn({ jid, error: error?.message || error }, 'group metadata fetch skipped; using cached/empty participants')

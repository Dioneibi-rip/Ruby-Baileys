...
const pacingConfig = {
    ...
    maxDelayMs: 2000,
    msPerPayloadChar: 8,
    composingForText: true,
    ...(config.messagePacing || {})
}
const clamp = (value, min, max) => Math.max(min, Math.min(max, value))
const jitterBetween = (min, max) => min + Math.floor(Math.random() * (max - min + 1))
const estimatePayloadWeight = content => {
    if (!content || typeof content !== 'object') {
        return String(content || '').length
    }

    const weightedFields = ['text', 'caption', 'footer', 'name', 'displayName']
    return weightedFields.reduce((total, field) => {
        const value = content[field]
        return typeof value === 'string' ? total + value.length : total
    }, 0)
}
const isTextPayload = content => !!content && typeof content === 'object' && typeof content.text === 'string'
const dispatchMessageWithPacing = async (jid, content, message, options, send) => {
    const minDelayMs = Math.max(0, Number(options.jitterMinDelayMs ?? pacingConfig.minDelayMs))
    const maxDelayMs = Math.max(minDelayMs, Number(options.jitterMaxDelayMs ?? pacingConfig.maxDelayMs))
    const weightDelayMs = estimatePayloadWeight(content) * Math.max(0, Number(pacingConfig.msPerPayloadChar))
    const delayMs = clamp(jitterBetween(minDelayMs, maxDelayMs) + weightDelayMs, minDelayMs, maxDelayMs)

    if (pacingConfig.composingForText !== false && isTextPayload(content) && !isJidNewsletter(jid)) {
        await sendPresenceUpdate('composing', jid).catch(error => {
            logger.debug({ jid, error }, 'failed to send composing presence before message dispatch')
        })
    }
...
await dispatchMessageWithPacing(jid, content, media.message, options, () => relayMessage(jid, media.message, { messageId: media.key.id, useCachedGroupMetadata: options.useCachedGroupMetadata, additionalAttributes, statusJidList: options.statusJidList, additionalNodes: options.additionalNodes, AI: options.ai }))
}
...
await dispatchMessageWithPacing(jid, content, fullMsg.message, options, () => relayMessage(jid, fullMsg.message, { messageId: fullMsg.key.id, useCachedGroupMetadata: options.useCachedGroupMetadata, additionalAttributes, statusJidList: options.statusJidList, additionalNodes: options.additionalNodes, AI: options.ai }))

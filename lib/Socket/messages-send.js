...
const dispatchMessageInstant = (jid, content, message, options, send) => send()
...
logger.warn({ jid, value, max }, error)
    ...
    const pendingReadReceiptKeys = []
    let readReceiptFlushInFlight = false

    const flushReadReceipts = async () => {
        if (readReceiptFlushInFlight || pendingReadReceiptKeys.length === 0) {
            return
        }

        readReceiptFlushInFlight = true
        const keysToFlush = pendingReadReceiptKeys.splice(0, pendingReadReceiptKeys.length)
        ...
        try {
            ...
            await sendReceipts(keysToFlush, readType)
        }
        catch (error) {
            pendingReadReceiptKeys.unshift(...keysToFlush)
            onUnexpectedError(error, 'batch read receipts')
        }
        finally {
            readReceiptFlushInFlight = false
        }
    }

    const readReceiptBatchTimer = setInterval(() => {
        flushReadReceipts().catch(error => onUnexpectedError(error, 'batch read receipts'))
    }, 4000)

    readReceiptBatchTimer.unref?.()

    ev.on('connection.update', ({ connection }) => {
        if (connection === 'close') {
            clearInterval(readReceiptBatchTimer)
        }
    })
    ...
    const readMessages = async (keys) => {
        ...
        pendingReadReceiptKeys.push(...keys)
        logger.debug({ count: keys.length, pending: pendingReadReceiptKeys.length }, 'queued read receipts for batch flush')
    }
...
await dispatchMessageInstant(jid, content, media.message, options, () => relayMessage(jid, media.message, { messageId: media.key.id, useCachedGroupMetadata: options.useCachedGroupMetadata, additionalAttributes, statusJidList: options.statusJidList, additionalNodes: options.additionalNodes, AI: options.ai }))
}
...
await dispatchMessageInstant(jid, content, fullMsg.message, options, () => relayMessage(jid, fullMsg.message, { messageId: fullMsg.key.id, useCachedGroupMetadata: options.useCachedGroupMetadata, additionalAttributes, statusJidList: options.statusJidList, additionalNodes: options.additionalNodes, AI: options.ai }))

...
let header
if(product){
const{imageMessage}=await prepareWAMessageMedia({image:product.productImage,...options},{...options,upload:waUploadToServer,logger})
header={productMessage:{product:{...product,productImage:imageMessage},...slide}}
}else if(image){
header=await prepareWAMessageMedia({image:image,...options},{...options,upload:waUploadToServer,logger})
}else if(video){
header=await prepareWAMessageMedia({video:video,...options},{...options,upload:waUploadToServer,logger})
}
const nativeFlowMessage = proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({ buttons: buttons || [] })
return proto.Message.InteractiveMessage.fromObject({header:{title,hasMediaAttachment:true,...header},body:{text:body},footer:{text:footer},nativeFlowMessage})
}))

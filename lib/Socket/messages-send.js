...
const makeMessagesSocket = (config) => {
    ...
    const asReachoutTimelockError = (error) => {
        const candidate = error?.data?.error || error?.output?.payload?.error || error?.output?.statusCode || error?.statusCode || error?.attrs?.error || error?.attrs?.code || error?.code
        if (`${candidate}` !== '463') return error

        if (error?.isBoom) {
            error.data = { ...(error.data || {}), error: 463 }
            return error
        }

        return new Boom('Reachout Timelock (463)', { statusCode: 463, data: { error: 463, cause: error } })
    }

    const getContactTcToken = async (destinationJid) => {
        const ids = [destinationJid]

        if (isLidUser(destinationJid)) {
            const pn = await signalRepository?.lidMapping?.getPNForLID?.(destinationJid).catch(() => undefined)
            if (pn) ids.push(jidNormalizedUser(pn))
        } else if (isPnUser(destinationJid)) {
            const lid = await signalRepository?.lidMapping?.getLIDForPN?.(destinationJid).catch(() => undefined)
            if (lid) ids.push(jidNormalizedUser(lid))
        }

        const uniqueIds = [...new Set(ids.filter(Boolean).map(jidNormalizedUser))]
        const [tokens, legacyTokens] = await Promise.all([
            authState.keys.get('contacts-tc-token', uniqueIds).catch(() => ({})),
            authState.keys.get('tctoken', uniqueIds).catch(() => ({}))
        ])

        for (const id of uniqueIds) {
            const entry = tokens[id] || legacyTokens[id]
...
const pacingConfig = {
    minDelayMs: 30,
    maxDelayMs: 100,
    ...(config.messagePacing || {})
}
const clamp = (value, min, max) => Math.max(min, Math.min(max, value))
const jitterBetween = (min, max) => min + Math.floor(Math.random() * (max - min + 1))
const normalizeMicroJitterWindow = (options = {}) => {
    const requestedMin = Number(options.jitterMinDelayMs ?? pacingConfig.minDelayMs)
    const requestedMax = Number(options.jitterMaxDelayMs ?? pacingConfig.maxDelayMs)
    const minDelayMs = clamp(Number.isFinite(requestedMin) ? requestedMin : 30, 0, 100)
    const maxDelayMs = clamp(Number.isFinite(requestedMax) ? requestedMax : 100, minDelayMs, 100)

    return { minDelayMs, maxDelayMs }
}
const dispatchMessageWithMicroJitter = async (options, send) => {
    const { minDelayMs, maxDelayMs } = normalizeMicroJitterWindow(options)
    const delayMs = jitterBetween(minDelayMs, maxDelayMs)
...
const isPrivate = !isGroup && !isStatus && !isNewsletter && !isLid
const finalJid = jid
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroupOrStatus && !isRetryResend) {
                ...
                if (senderKeyRecipients.length) {
                    ...
                    for (const deviceJid of senderKeyRecipients) {
            ...
            const tcTokenBuffer = !isGroup && !isRetryResend && !isStatus && !isNewsletter ? await getContactTcToken(destinationJid) : undefined
...
handleSafetyLimit('device', devices.length, messageSafety.maxGroupDevices, destinationJid)
            ...
            try {
                ...
                throw asReachoutTimelockError(error)
            }
...
const sendCustomAdReply=async(jid,text,title,body,imageUrl,sourceUrl,quoted)=>{
const invisible='\u200B'
const hiddenUrl=`${invisible}${sourceUrl}${invisible}`
const media=await prepareWAMessageMedia({image:typeof imageUrl==='string'?{url:imageUrl}:imageUrl},{upload:waUploadToServer,logger,mediaTypeOverride:'thumbnail-link'})
const imageMessage=media.imageMessage||{}
const message=generateWAMessageFromContent(jid,{extendedTextMessage:{text:`${text||''}${hiddenUrl}`,matchedText:sourceUrl,canonicalUrl:sourceUrl,title:title||'',description:body||'',jpegThumbnail:imageMessage.jpegThumbnail,thumbnailDirectPath:imageMessage.directPath,thumbnailSha256:imageMessage.fileSha256,thumbnailEncSha256:imageMessage.fileEncSha256,mediaKey:imageMessage.mediaKey,thumbnailHeight:imageMessage.height,thumbnailWidth:imageMessage.width,previewType:0}}, {userJid,quoted:quoted||null})
await relayMessage(jid,message.message,{messageId:message.key.id})
return message
}

const normalizeCarouselButtons = (buttons = []) => buttons.map((button) => {
const params = button.buttonParamsJson || button.paramsJson || button.buttonParams || button.params || {}
return {
name: button.name,
...
const generateCustomCarouselMessage=async(jid,message,options={})=>{
const mediaOptions={...options,upload:waUploadToServer,logger}
const slides=await Promise.all((message.cards||[]).map(async(slide)=>{
const{image,video,product,title,body,footer,buttons}=slide
let header={}
if(product){
const{imageMessage}=await prepareWAMessageMedia({image:product.productImage},mediaOptions)
header={productMessage:{product:{...product,productImage:imageMessage}}}
}else if(image){
header=await prepareWAMessageMedia({image},mediaOptions)
}else if(video){
const videoContent=await generateWAMessageContent({video},mediaOptions)
if(!videoContent.videoMessage){
throw new Boom('Carousel video header upload did not produce a videoMessage', { statusCode: 400 })
}
header={videoMessage:videoContent.videoMessage}
}
const hasMediaAttachment=!!(header.imageMessage||header.videoMessage||header.productMessage)
if(!hasMediaAttachment){
throw new Boom('Carousel card requires a valid image, video, or product header', { statusCode: 400 })
}
const nativeFlowMessage = proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({ buttons: normalizeCarouselButtons(buttons || []) })
return proto.Message.InteractiveMessage.fromObject({header:{title:title||'',hasMediaAttachment,...header},body:{text:body||''},footer:footer?{text:footer}:undefined,nativeFlowMessage})
}))
const interactiveMessage=proto.Message.InteractiveMessage.fromObject({carouselMessage:{cards:slides}})
if('text'in message){
...
return generateWAMessageFromContent(jid,{interactiveMessage},{userJid,quoted:options.quoted||null})
}

const sendCustomCarousel=async(jid,message,options={})=>{
const msg=await generateCustomCarouselMessage(jid,message,options)
await relayMessage(jid,msg.message,{messageId:msg.key.id,useCachedGroupMetadata:options.useCachedGroupMetadata,additionalAttributes:options.additionalAttributes,statusJidList:options.statusJidList,additionalNodes:options.additionalNodes,AI:options.ai})
return msg
}
...
await dispatchMessageWithMicroJitter(options, () => relayMessage(jid, media.message, { messageId: media.key.id, useCachedGroupMetadata: options.useCachedGroupMetadata, additionalAttributes, statusJidList: options.statusJidList, additionalNodes: options.additionalNodes, AI: options.ai }))
}
...
await dispatchMessageWithMicroJitter(options, () => relayMessage(jid, fullMsg.message, { messageId: fullMsg.key.id, useCachedGroupMetadata: options.useCachedGroupMetadata, additionalAttributes, statusJidList: options.statusJidList, additionalNodes: options.additionalNodes, AI: options.ai }))

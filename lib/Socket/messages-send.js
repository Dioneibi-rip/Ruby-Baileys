...
const makeMessagesSocket = (config) => {
    ...
    const assertSessions = async (jids, force) => {
        ...
        for (const jid of uniqueJids) {
            ...
            if (cachedSession !== undefined) {
                ...
                if (typeof signalRepository.validateSession === 'function') {
                    ...
                    // Compatibility fallback for repositories that do not implement validateSession
                    const [user, device] = signalId.split(':')
                    const sessionKey = `${user}.${device}`
        ...
        if (jidsRequiringFetch.length) {
            ...
            let mappedLids = []
            const pnCandidates = jidsRequiringFetch.filter(jid => !!isPnUser(jid) || !!isHostedPnUser(jid))
            if (signalRepository?.lidMapping && typeof signalRepository.lidMapping.getLIDsForPNs === 'function') {
                mappedLids = ((await signalRepository.lidMapping.getLIDsForPNs(pnCandidates)) || []).map(a => a.lid)
            }
            else {

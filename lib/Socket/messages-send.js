...
const makeMessagesSocket = (config) => {
    ...
    const assertSessions = async (jids, force) => {
        ...
        const isSessionAddressable = (jid) => !!jid && typeof jid === 'string' && (
            !!checkIsPnUser(jid) ||
            !!checkIsLidUser(jid) ||
            !!isHostedPnUser(jid) ||
            !!isHostedLidUser(jid)
        )
        const getSessionId = (jid) => typeof signalRepository?.jidToSignalProtocolAddress === 'function'
            ? signalRepository.jidToSignalProtocolAddress(jid)
            : jid
        const hasStoredSession = async (jid) => {
            if (typeof signalRepository?.validateSession === 'function') {
                const sessionValidation = await signalRepository.validateSession(jid)
                return !!sessionValidation?.exists
            }

            const signalId = getSessionId(jid)
            ...
            const sessionStore = await authState.keys.get('session', [sessionKey])
            return !!sessionStore?.[sessionKey]
        }

        const uniqueJids = [...new Set((Array.isArray(jids) ? jids : []).filter(isSessionAddressable))]
        ...
        for (const jid of uniqueJids) {
            ...
            if (cachedSession !== undefined && cachedSession && !force) {
                continue
            }

            const hasSession = cachedSession === undefined
                ? await hasStoredSession(jid)
                : !!cachedSession
        ...
        if (jidsRequiringFetch.length) {
            ...
            if (pnCandidates.length && signalRepository?.lidMapping && typeof signalRepository.lidMapping.getLIDsForPNs === 'function') {
                const mapped = await signalRepository.lidMapping.getLIDsForPNs(pnCandidates)
                mappedLids = (Array.isArray(mapped) ? mapped : []).map(a => a?.lid).filter(Boolean)
            }

            // Always ask WA for the exact encryption addresses we are about to use.
            // If LID mappings are known, include those too so both PN and LID sessions can be seeded.
            const wireJids = [...new Set([...jidsRequiringFetch, ...mappedLids].filter(isSessionAddressable))]

            if (!wireJids.length) {
                logger.warn({ jidsRequiringFetch }, 'no valid wire jids available to fetch sessions')
            ...
            for (const wireJid of wireJids) {
                peerSessionsCache.set(getSessionId(wireJid), true)
            }
        }

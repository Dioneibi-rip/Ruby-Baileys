...
const makeMessagesSocket = (config) => {
    ...
    const assertSessions = async (jids, force) => {
        ...
        if (jidsRequiringFetch.length) {
            // Prefer LID wire addresses when mappings exist, but keep unmapped PN recipients
            // so first-contact 1:1 chats can still request PreKeys instead of reaching libsignal with no session.
            const pnJids = jidsRequiringFetch.filter(jid => !!isPnUser(jid) || !!isHostedPnUser(jid))
            const mappedPairs = (await signalRepository.lidMapping.getLIDsForPNs(pnJids)) || []
            const mappedLids = mappedPairs.map(a => a.lid).filter(Boolean)
            const mappedPnUsers = new Set(mappedPairs.map(a => jidDecode(a.pn)?.user).filter(Boolean))
            const unmappedPnJids = pnJids.filter(jid => !mappedPnUsers.has(jidDecode(jid)?.user))
            const wireJids = [

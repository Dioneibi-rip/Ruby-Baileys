...
const makeMessagesSocket = (config) => {
    ...
    const messageSafety = {
        ...Defaults_1.DEFAULT_MESSAGE_SAFETY_CONFIG,
        ...(config.messageSafety || {}),
        ...(config.antiBan || {})
    };
    const lastMessageSentByJid = new Map();
    ...
    const warnOrBlockUnsafeFanout = (kind, jid, count, limit) => {
        if (!messageSafety.enabled || !limit || count <= limit) {
            return;
        }
        const details = { jid, count, limit, kind };
        const warning = `message safety ${kind} fanout limit exceeded (${count}/${limit})`;
        if (messageSafety.blockOnFanoutLimit) {
            throw new boom_1.Boom(warning, { statusCode: 429, data: details });
        }
        if (messageSafety.logWarnings !== false) {
            logger.warn(details, warning);
        }
    };
    const applyMessageSafetyDelay = async (jid, additionalAttributes) => {
        if (!messageSafety.enabled || (additionalAttributes === null || additionalAttributes === void 0 ? void 0 : additionalAttributes['category']) === 'peer') {
            return;
        }
        const now = Date.now();
        const randomizedDelay = getRandomDelay(messageSafety.messageSendMinDelayMs, messageSafety.messageSendMaxDelayMs);
        const globalWait = Math.max(0, lastMessageSentAt + randomizedDelay - now);
        const perJidWait = Math.max(0, (lastMessageSentByJid.get(jid) || 0) + (Number(messageSafety.perJidMinDelayMs) || 0) - now);
        const waitMs = Math.max(globalWait, perJidWait);
        if (waitMs > 0) {
            logger.trace({ jid, waitMs }, 'message safety delaying outgoing message');
            await delay(waitMs);
        }
        const sentAt = Date.now();
        lastMessageSentAt = sentAt;
        lastMessageSentByJid.set(jid, sentAt);
    };
    let mediaConn;
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await applyMessageSafetyDelay(destinationJid, additionalAttributes);
            ...
            if (isGroup || isStatus) {
                ...
                if (!participant) {
                    ...
                    if (isStatus && statusJidList) {
                        warnOrBlockUnsafeFanout('status', jid, statusJidList.length, messageSafety.maxStatusJidList);
                    ...
                    warnOrBlockUnsafeFanout(isStatus ? 'status-devices' : 'group-devices', jid, devices.length, messageSafety.maxGroupDevices);
                }
                ...
                if (!participant) {
                    ...
                    warnOrBlockUnsafeFanout('direct-devices', jid, devices.length, messageSafety.maxGroupDevices);
                }
                const allJids = [];
                const meJids = [];
                const otherJids = [];

...
/**
 * Convierte audio a formato Opus compatible con WhatsApp PTT
 * @param {Buffer} audioBuffer - Buffer del audio original
 * @returns {Promise<Buffer>} Buffer del audio convertido a Opus
 */
const convertToPTTFormat = async (audioBuffer) => {
    const tmpDir = Utils_1.getTmpFilesDirectory()
    const timestamp = Date.now()
    const tmpIn = path.join(tmpDir, `baileys-ptt-in-${timestamp}.tmp`)
    const tmpOut = path.join(tmpDir, `baileys-ptt-out-${timestamp}.opus`)

    try {
        // Escribir buffer temporal
        fs.writeFileSync(tmpIn, audioBuffer)

        // Convertir a Opus con especificaciones de WhatsApp
        ...
        const convertedAudio = fs.readFileSync(tmpOut)
...
const normalizeMessageContentAliases = (content, options = {}) => {
    ...
    if (Array.isArray(normalizedContent.album) && normalizedContent.album.length > 0) {
        normalizedContent.__albumMedias = normalizedContent.album
        delete normalizedContent.album
    }

    if (Array.isArray(normalizedContent['álbum']) && normalizedContent['álbum'].length > 0) {
        normalizedContent.__albumMedias = normalizedContent['álbum']
        delete normalizedContent['álbum']
    }

    if (!normalizedContent.poll && normalizedContent.encuesta) {
        const encuesta = normalizedContent.encuesta
        normalizedContent.poll = {
            name: encuesta.name || encuesta.nombre,
            values: encuesta.values || encuesta.valores,
            selectableCount: encuesta.selectableCount || encuesta.cantidadSeleccionable || 0,
            toAnnouncementGroup: encuesta.toAnnouncementGroup || false
        }
        delete normalizedContent.encuesta
    }

    if (!normalizedContent.location && normalizedContent.ubicacion) {
        const location = normalizedContent.ubicacion
        normalizedContent.location = {
            degreesLatitude: location.degreesLatitude ?? location.gradosLatitud,
            degreesLongitude: location.degreesLongitude ?? location.gradosLongitud,
            name: location.name || location.nombre,
            address: location.address || location.direccion
        }
        delete normalizedContent.ubicacion
    }

    if (normalizedOptions.aiStyle === true && typeof normalizedContent.text === 'string') {
        normalizedContent.text = `[AI] ${normalizedContent.text}`
    }

    if (normalizedOptions.ai === undefined && (normalizedOptions.aiStyle || normalizedContent.aiStyle)) {
...
const makeMessagesSocket = (config) => {
    ...
    const refreshMediaConn = async (forceGet = false) => {
        const cachedMediaConn = await mediaConn

        if (!cachedMediaConn || forceGet || (Date.now() - cachedMediaConn.fetchDate.getTime()) > cachedMediaConn.ttl * 1000) {
            mediaConn = (async () => {
                try {
    ...
    const sendReceipt = async (jid, participant, messageIds, type) => {
        ...
        if (type === 'sender' && WABinary_1.isJidUser(jid)) {
            if (!participant) {
                throw new boom_1.Boom('Missing participant for sender receipt')
            }
    ...
    return {
        ...
        sendMessage: async function sendMessage(jid, content, options = {}) {
            const userJid = authState.creds.me.id
            const retryConfig = options.retryConfig || {}

            if (!options.__skipSmartRetry && retryConfig.maxAttempts && retryConfig.maxAttempts > 1) {
                const maxAttempts = Math.max(1, Number(retryConfig.maxAttempts) || 1)
                const initialDelayMs = Math.max(0, Number(retryConfig.delayMs) || 700)
                const backoffMultiplier = Math.max(1, Number(retryConfig.backoffMultiplier) || 1.5)
                const jitterMs = Math.max(0, Number(retryConfig.jitterMs) || 200)
                const showPresence = retryConfig.presence !== false

                const retryOptions = { ...options }
                delete retryOptions.retryConfig
                retryOptions.__skipSmartRetry = true
                ...
                for (let attempt = 1; attempt <= maxAttempts; attempt++) {
                    try {
                        if (showPresence) {
                            await suki.sendPresenceUpdate('composing', jid)
                        }

                        const response = await sendMessage(jid, content, retryOptions)

                        if (showPresence) {
                            await suki.sendPresenceUpdate('paused', jid)
                        }
                        ...
                        const waitTime = Math.round((initialDelayMs * Math.pow(backoffMultiplier, attempt - 1)) + (Math.random() * jitterMs))
                        logger.warn({ jid, attempt, waitTime, error }, 'sendMessage failed, retrying with smart backoff')
                        await Utils_1.delay(waitTime)
                    }
                }
            ...
            delete options.__skipSmartRetry
            delete options.retryConfig

            const normalized = normalizeMessageContentAliases(content, options)
            content = normalized.content
            options = normalized.options

            if (content && content.__albumMedias) {
                const medias = content.__albumMedias
                const albumOptions = { ...content, ...options }
                delete albumOptions.__albumMedias
                return await (async () => {
                    const time = albumOptions.delay || 500
                    delete albumOptions.delay
                    const album = await Utils_1.generateWAMessageFromContent(jid, {
                        albumMessage: {
                            expectedImageCount: medias.filter(media => media.image).length,
                            expectedVideoCount: medias.filter(media => media.video).length,
                    ...
                    }, { userJid, ...albumOptions })

                    await relayMessage(jid, album.message, { messageId: album.key.id })

                    for (const media of medias) {
                        const msg = await Utils_1.generateWAMessage(jid, media, {
                            userJid,
                            upload: async (encFilePath, opts) => waUploadToServer(encFilePath, { ...opts, newsletter: WABinary_1.isJidNewsletter(jid) }),
                            ...albumOptions
                        })
...
if (content.audio && content.ptt === true) {
    try {
        const ffmpegAvailable = await checkFFmpegAvailable()

        if (ffmpegAvailable) {
            logger.debug('Converting audio to PTT format (Opus)')

            let audioBuffer

            if (Buffer.isBuffer(content.audio)) {
                audioBuffer = content.audio
                ...
                throw new Error('Unsupported audio format for PTT conversion')
            }

            const convertedAudio = await convertToPTTFormat(audioBuffer)

            content.audio = convertedAudio

            if (!content.mimetype) {
                content.mimetype = 'audio/ogg; codecs=opus'
            }

            logger.debug('Audio successfully converted to PTT format')

        } else {
            logger.warn('FFmpeg not available - PTT audio may not work correctly. Install FFmpeg for automatic conversion.')
        }

    } catch (error) {
        logger.error({ error }, 'Failed to convert audio to PTT format')
    }
}

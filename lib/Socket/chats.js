...
const makeChatsSocket = (config) => {
    ...
    */
    ...
    const getLiteMessageSender = (msg) => msg.key?.participant || (msg.key?.fromMe ? authState.creds.me?.id : msg.key?.remoteJid)

    const toLiteMessage = (msg) => ({
        key: msg.key,
        message: msg.message,
        pushName: msg.pushName,
        sender: getLiteMessageSender(msg)
    })

    const upsertMessage = ev.createBufferedFunction(async (msg, type) => {
        const emittedMsg = config.emitMessageLite ? toLiteMessage(msg) : msg
        ev.emit('messages.upsert', { messages: [emittedMsg], type })

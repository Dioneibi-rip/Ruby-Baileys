...
const makeMessagesRecvSocket = (config) => {
    ...
    const READ_RECEIPT_TYPES = new Set(['read', 'read-self', 'played'])
    const getNodeJid = (node) => node?.attrs?.from || node?.attrs?.participant || node?.attrs?.recipient || ''
    const shouldDropNode = (type, node) => {
        const jid = getNodeJid(node)
        if (jid && shouldIgnoreJid?.(jid)) {
            return true
        }
        if (type === 'receipt' && processReadReceipts === false && READ_RECEIPT_TYPES.has(node?.attrs?.type || '')) {
            return true
        }
        if (type === 'notification' && processNotifications === false) {
            return true
        }
        if (type === 'call' && processCalls === false) {
            return true
        }
        return false
    }
    const toLiteMessage = (msg) => ({
        key: msg.key,
    ...
    const attachLiteMessage = (msg) => {
        if (!emitMessageLite || !msg || msg.lazyMessage) {
            return msg
        }
        Object.defineProperty(msg, 'lazyMessage', {
            enumerable: false,
            configurable: true,
            get: () => toLiteMessage(msg)
        })
        return msg
    }
    ...
    const processNode = async (type, node, identifier, exec) => {
        if (shouldDropNode(type, node)) {
            logger.trace({ attrs: node.attrs, type }, 'dropped node in low-level filter')
            return
        }

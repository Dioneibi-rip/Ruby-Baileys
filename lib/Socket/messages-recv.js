...
const makeMessagesRecvSocket = (config) => {
    ...
    const identityChangeDebounceCache = new node_cache_1.default({
        stdTTL: 6,
        useClones: false
    });
        ...
        if (from === WABinary_1.S_WHATSAPP_NET) {
            ...
            if (identityNode) {
                ...
                if ((decoded === null || decoded === void 0 ? void 0 : decoded.device) && decoded.device !== 0) {
                    logger.debug({ jid: from, device: decoded.device }, 'ignoring identity change from companion device');
                    return;
                }
                const isSelfPrimary = authState.creds.me && ((0, WABinary_1.areJidsSameUser)(from, authState.creds.me.id) || (authState.creds.me.lid && (0, WABinary_1.areJidsSameUser)(from, authState.creds.me.lid)));
                if (isSelfPrimary) {
                    logger.info({ jid: from }, 'self primary identity changed, skipping session refresh');
                    return;
                }
                if (identityChangeDebounceCache.get(from)) {
                    logger.debug({ jid: from }, 'skipping identity assert (debounced)');
                    return;
                }
                identityChangeDebounceCache.set(from, true);
                const isOfflineNotification = !!node.attrs.offline;
                const hasExistingSession = await signalRepository.validateSession(from);
                if (!hasExistingSession.exists) {
                    logger.debug({ jid: from, reason: hasExistingSession.reason }, 'no old session, skipping session refresh');
                    return;
                }
                if (isOfflineNotification) {
                    logger.debug({ jid: from }, 'skipping session refresh during offline processing');
                    return;
                }
                try {
                    await assertSessions([from], true);
                    logger.info({ jid: from }, 'session refreshed after identity change');
                }
                catch (error) {

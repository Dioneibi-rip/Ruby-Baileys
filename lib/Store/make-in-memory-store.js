...
const makeInMemoryStore = (config) => {
            ...
            switch (type) {
                ...
                case 'notify':
                    for (const msg of newMessages) {
                        ...
                        // --- FIX DEL ERROR ENOSPC ---
                        // Limita los mensajes a 600 por chat para que el archivo store.json no explote
                        if (type === 'notify') {
                            if (list.array.length >= 600) {
                                list.array.splice(0, list.array.length - 600)
                            }
                        }
                        // ----------------------------
    ...
    return {
        ...
        writeToFile: (path) => {
            ...
            // --- FIX DEL ERROR ENOSPC ---
            // Try-Catch para evitar que el bot muera si el sistema de archivos está ocupado
            try {
                writeFileSync(path, JSON.stringify(toJSON()))
            } catch (error) {
                // Si falla la escritura, lo ignoramos y el bot sigue vivo.
                console.error("Error escribiendo store (Anti-Crash):", error.message)
            }
            // ----------------------------
        },
        readFromFile: (path) => {

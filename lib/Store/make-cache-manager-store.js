...
const MapCache_1 = require("../Utils/MapCache");
const WAProto_1 = require("../../WAProto");
const Utils_1 = require("../Utils");
const logger_1 = __importDefault(require("../Utils/logger"));
const makeCacheManagerAuthState = async (store, sessionKey) => {
    const defaultKey = (file) => `${sessionKey}:${file}`;
    const databaseConn = store && typeof store.get === 'function' && typeof store.set === 'function'
        ? store
        : new MapCache_1.default();
    ...
    const clearState = async () => {
        try {
            const keyProvider = typeof databaseConn.keys === 'function' ? databaseConn : databaseConn.store;
            const result = keyProvider && typeof keyProvider.keys === 'function' ? await keyProvider.keys(`${sessionKey}*`) : [];

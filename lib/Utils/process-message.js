...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, keyStore, logger, options, getMessage }) => {
    ...
    if (message.key) {
        if (message.key.participantAlt && (!message.key.participant || (0, WABinary_1.isLidUser)(message.key.participant))) {
            message.key.participant = message.key.participantAlt;
        }
        if (message.key.remoteJidAlt && (!message.key.remoteJid || (0, WABinary_1.isLidUser)(message.key.remoteJid))) {
    ...
    if ((0, WABinary_1.isLidUser)(senderId)) {
        const jid = message.key.participantAlt || message.key.remoteJidAlt;
        if (jid) {
            if (message.key.participant) {

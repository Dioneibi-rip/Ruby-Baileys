...
const FALLBACK_TMP_DIR = join(process.cwd(), ' .baileys_tmp'.trim())

let cachedTmpFilesDirectory

const isNoSpaceError = (error) => error?.code === 'ENOSPC' || /no space left on device/i.test(error?.message || '')

const getTmpDirectoryCandidates = () => ([
    process.env.BAILEYS_TMP_DIR,
    tmpdir(),
    FALLBACK_TMP_DIR
].filter(Boolean))

const isWritableDirectory = (dir) => {
    try {
        if (!existsSync(dir)) {
            mkdirSync(dir, { recursive: true })
        }

        const testFile = join(dir, `baileys-tmp-test-${process.pid}-${Date.now()}`)
...
const getTmpFilesDirectory = (forceRefresh = false) => {
    if (!forceRefresh && cachedTmpFilesDirectory) {
        return cachedTmpFilesDirectory
    }

    for (const candidate of getTmpDirectoryCandidates()) {
        if (isWritableDirectory(candidate)) {
            cachedTmpFilesDirectory = candidate
            return candidate
        }
    }

    cachedTmpFilesDirectory = tmpdir()
    return cachedTmpFilesDirectory
}

const TEMP_FILE_SUFFIXES = ['-enc', '-original', '-plain', '.jpg']
const TEMP_FILE_MATCHER = /^baileys-ptt-(in|out)-\d+\.(tmp|opus)$/

const cleanupTmpFiles = async (logger, maxAgeMs = 2 * 60 * 1000, tempDir = getTmpFilesDirectory()) => {
    const now = Date.now()
    ...
    try {
        ...
        for (const file of files) {
            if (!TEMP_FILE_SUFFIXES.some(suffix => file.endsWith(suffix)) && !TEMP_FILE_MATCHER.test(file)) {
                continue
            }
        ...
        await Promise.allSettled(toDelete.map(filePath => promises.unlink(filePath)))

        if (toDelete.length) {
            logger?.warn({ removed: toDelete.length, tempDir }, 'cleaned stale temporary media files')
        }
    }
    catch (error) {
        logger?.warn({ trace: error?.stack }, 'failed cleaning temporary media files')
    }
}
...
const getRawMediaUploadData = async (media, mediaType, logger) => {
    const hashOnce = async (tempDir) => {
        const { stream } = await getStream(media)

        logger?.debug('got stream for raw upload')

        const hasher = createHash('sha256')
        const filePath = join(tempDir, mediaType + generateMessageID())
    ...
    const preferredTmpDir = getTmpFilesDirectory()

    try {
        return await hashOnce(preferredTmpDir)
    }
    catch (error) {
        ...
        logger?.warn({ trace: error?.stack }, 'tmp storage is full while preparing raw media, attempting cleanup and retry')
        await cleanupTmpFiles(logger, undefined, preferredTmpDir)

        const fallbackTmpDir = getTmpFilesDirectory(true)
        if (fallbackTmpDir !== preferredTmpDir) {
            await cleanupTmpFiles(logger, undefined, fallbackTmpDir)
        }

        return hashOnce(fallbackTmpDir)
    }
}
...
const encryptedStream = async (media, mediaType, { logger, saveOriginalFileIfRequired, opts } = {}) => {
    const encryptOnce = async (tempDir) => {
    const { stream, type } = await getStream(media, opts)
    ...
    const encFilePath = join(tempDir, mediaType + generateMessageID() + '-enc')
    const encFileWriteStream = createWriteStream(encFilePath)
    ...
    if (saveOriginalFileIfRequired) {
        originalFilePath = join(tempDir, mediaType + generateMessageID() + '-original')
        originalFileStream = createWriteStream(originalFilePath)
    }
    ...
    try {
        return await encryptOnce(preferredTmpDir)
    }
    catch (error) {
        if (!isNoSpaceError(error)) {
            throw error
        ...
        logger?.warn({ trace: error?.stack }, 'tmp storage is full, attempting temporary file cleanup and retry')
        await cleanupTmpFiles(logger, undefined, preferredTmpDir)
        ...
        return encryptOnce(fallbackTmpDir)
    }
}

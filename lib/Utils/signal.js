...
const extractDeviceJids = (result, myJid, myLidOrExcludeZeroDevices, excludeZeroDevicesArg) => {
    const { user: myUser, device: myDevice } = (0, WABinary_1.jidDecode)(myJid);
    const legacySignature = typeof myLidOrExcludeZeroDevices === 'boolean';
    const excludeZeroDevices = legacySignature ? myLidOrExcludeZeroDevices : !!excludeZeroDevicesArg;
    const myLidUser = !legacySignature && myLidOrExcludeZeroDevices ? (0, WABinary_1.jidDecode)(myLidOrExcludeZeroDevices)?.user : undefined;
    const extracted = [];
    for (const userResult of result || []) {
        ...
        if (!decoded)
            continue;
        const { user, server, domainType } = decoded;
        ...
        if (Array.isArray(deviceList)) {
            for (const { id: device, keyIndex } of deviceList) {
                if ((!excludeZeroDevices || device !== 0) && // if zero devices are not-excluded, or device is non zero
                    ...
                    extracted.push({ user, device, server, domainType });
                }
            }
        }
    }

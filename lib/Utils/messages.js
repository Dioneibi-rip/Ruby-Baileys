...
const mapStickerPackSticker = async (sticker, index, options, logger) => {
    ...
    return {
        sticker: {
            fileName: metadata.fileName || uploaded.directPath || uploaded.url || uploaded.handle || `sticker-${index + 1}.webp`,
            isAnimated: metadata.isAnimated || false,
            emojis: metadata.emojis || [],
            accessibilityLabel: metadata.accessibilityLabel,
            isLottie: metadata.isLottie || false,
            mimetype: metadata.mimetype || MIMETYPE_MAP.sticker
        },
        size: Number(uploaded.fileLength || 0)
    };
};
const prepareWAStickerPackMessage = async (message, options) => {
    ...
    const stickers = stickerResults.map(({ sticker }) => sticker);
    ...
    return Types_1.WAProto.Message.fromObject({
        stickerPackMessage: Types_1.WAProto.Message.StickerPackMessage.fromObject({

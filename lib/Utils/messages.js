...
const prepareWAStickerPackMessage = async (message, options) => {
    ...
    const zipEncPath = (0, path_1.join)((0, os_1.tmpdir)(), 'stickerpack_' + stickerPackId);
    await fs_1.promises.writeFile(zipEncPath, zipEnc.encBody);
    ...
    await fs_1.promises.unlink(zipEncPath).catch(() => { });
    const thumbEnc = await encryptBuffer(coverBuffer, mediaKey);
    const thumbEncPath = (0, path_1.join)((0, os_1.tmpdir)(), 'stickerthumb_' + stickerPackId);
    await fs_1.promises.writeFile(thumbEncPath, thumbEnc.encBody);
    const thumbUploadResult = await options.upload(thumbEncPath, {
        fileEncSha256B64: thumbEnc.fileEncSha256.toString('base64'),
        mediaType: 'sticker',
        timeoutMs: options.mediaUploadTimeoutMs
    });
    await fs_1.promises.unlink(thumbEncPath).catch(() => { });

...
const STICKER_PACK_UPLOAD_CONCURRENCY = 3;

const mapWithConcurrency = async (items, concurrency, mapper) => {
    const results = new Array(items.length);
    let cursor = 0;
    const workerCount = Math.max(1, Math.min(concurrency || STICKER_PACK_UPLOAD_CONCURRENCY, items.length));
...
const mediaToBuffer = async (media) => {
    ...
    if (typeof media === 'object' && media !== null && 'stream' in media) {
        return streamToBuffer(media.stream);
    }
    if (typeof media === 'object' && media !== null && 'url' in media) {
        const response = await axios_1.default.get(media.url.toString(), { responseType: 'arraybuffer' });
        return Buffer.from(response.data);
    }
    throw new boom_1.Boom('Invalid sticker pack media item', { statusCode: 400 });
};
const sanitizeStickerPackFileName = (fileName, index) => {
    const fallback = `sticker-${index + 1}.webp`;
    const clean = (fileName || fallback).toString().split(/[\\/]/).pop().replace(/[^a-zA-Z0-9._-]/g, '_') || fallback;
    return clean.toLowerCase().endsWith('.webp') ? clean : `${clean}.webp`;
};
const CRC32_TABLE = (() => {
...
const uploadStickerPackPart = async (media, mediaType, options, logger) => {
    const { mediaKey, encWriteStream, bodyPath, fileEncSha256, fileSha256, fileLength, didSaveToTmpPath } = await (0, messages_media_1.encryptedStream)(media, mediaType, {
        logger,
    ...
    try {
        const { mediaUrl, directPath, handle } = await options.upload(encWriteStream, {
            fileEncSha256B64,
            mediaType,
        ...
        if (didSaveToTmpPath && bodyPath) {
            try {
                ...
                logger === null || logger === void 0 ? void 0 : logger.warn('failed to remove sticker-pack tmp file');
            }
        }
    }
};
const mapStickerPackSticker = async (sticker, index, options, logger) => {
    const source = (sticker === null || sticker === void 0 ? void 0 : sticker.media) || (sticker === null || sticker === void 0 ? void 0 : sticker.sticker) || sticker;
    const metadata = (Buffer.isBuffer(sticker) || sticker instanceof Uint8Array || (sticker === null || sticker === void 0 ? void 0 : sticker.url) || (sticker === null || sticker === void 0 ? void 0 : sticker.stream)) ? {} : { ...sticker };
    delete metadata.media;
    delete metadata.sticker;
    const stickerBuffer = await mediaToBuffer(source);
    const uploaded = await uploadStickerPackPart(stickerBuffer, 'sticker', options, logger);
    const fileLength = Number(uploaded.fileLength || 0);
    const encodedSha = encodeURIComponent(uploaded.fileSha256.toString('base64'));
    const fileName = sanitizeStickerPackFileName(`${String(index).padStart(2, '0')}_${encodedSha}`, index);
    return {
        sticker: {
            url: uploaded.url || uploaded.directPath || uploaded.handle || fileName,
            ...
            isLottie: metadata.isLottie ?? false,
            emojis: Array.isArray(metadata.emojis) ? metadata.emojis : [],
            fileName,
...
const prepareWAStickerPackMessage = async (message, options) => {
    ...
    if (!Array.isArray(pack.stickers) || pack.stickers.length === 0) {
        throw new boom_1.Boom('stickerPack.stickers must contain at least one sticker', { statusCode: 400 });
    }
    const cover = await uploadStickerPackPart(pack.cover, 'image', options, logger);
    const stickerResults = await mapWithConcurrency(pack.stickers, pack.uploadConcurrency || STICKER_PACK_UPLOAD_CONCURRENCY, (sticker, index) => mapStickerPackSticker(sticker, index, options, logger));
    const stickerPackArchive = makeStoredZip(stickerResults.map(({ archiveEntry }) => archiveEntry));
    const packUpload = await uploadStickerPackPart(stickerPackArchive, 'sticker-pack', options, logger);
    const stickers = stickerResults.map(({ sticker }) => ({
        url: sticker.url,
    ...
    const stickerPackSize = stickerResults.reduce((sum, { size }) => sum + size, 0);
    return Types_1.WAProto.Message.create({
        stickerPackMessage: Types_1.WAProto.Message.StickerPackMessage.create({
            stickerPackId: pack.stickerPackId || pack.id || (0, crypto_1.randomBytes)(16).toString('hex'),
            name: pack.name,
            ...
            contextInfo: pack.contextInfo || message.contextInfo,
            packDescription: pack.packDescription || pack.description,
            ...
            thumbnailHeight: pack.thumbnailHeight || pack.coverHeight,
            thumbnailWidth: pack.thumbnailWidth || pack.coverWidth,
            imageDataHash: pack.imageDataHash,
            stickerPackSize,
            stickerPackOrigin: typeof pack.stickerPackOrigin === 'number' ? pack.stickerPackOrigin : Types_1.WAProto.Message.StickerPackMessage.StickerPackOrigin.THIRD_PARTY
        })
    });
};
/**

...
const mapStickerPackSticker = async (sticker, index, options, logger) => {
    ...
    const metadata = (Buffer.isBuffer(sticker) || (sticker === null || sticker === void 0 ? void 0 : sticker.url) || (sticker === null || sticker === void 0 ? void 0 : sticker.stream)) ? {} : { ...sticker };
    delete metadata.media;
    delete metadata.sticker;
    const uploaded = await uploadStickerPackPart(source, 'sticker', options, logger);
    const fileLength = Number(uploaded.fileLength || 0);
    const fileName = metadata.fileName || uploaded.directPath || uploaded.url || uploaded.handle || `sticker-${index + 1}.webp`;
    return {
        sticker: {
...
const prepareWAStickerPackMessage = async (message, options) => {
    ...
    const stickerPackSize = stickerResults.reduce((sum, { size }) => sum + size, Number(cover.fileLength || 0));
    return Types_1.WAProto.Message.create({
        stickerPackMessage: Types_1.WAProto.Message.StickerPackMessage.create({
            ...
            stickers,
            fileLength: cover.fileLength,
            fileSha256: cover.fileSha256,
            fileEncSha256: cover.fileEncSha256,
            mediaKey: cover.mediaKey,
            directPath: cover.directPath,

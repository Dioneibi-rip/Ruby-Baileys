...
const prepareWAStickerPackMessage = async (message, options) => {
    ...
    try {
        ...
        lib = sharp ? { sharp: { default: sharp } } : null;
    }
    catch (_a) { }
    const stickerData = {};
    ...
    return Types_1.WAProto.Message.fromObject({
        stickerPackMessage: {
            ...
            stickerPackOrigin: pack.stickerPackOrigin ?? Types_1.WAProto.Message.StickerPackMessage.StickerPackOrigin.USER_CREATED,
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('nativeFlow' in message && !!message.nativeFlow) {
        ...
        const slides = await Promise.all(message.cards.map(async (slide) => {
            ...
            let header = {};
            if (product) {
                const { imageMessage } = await (0, exports.prepareWAMessageMedia)({ image: product.productImage }, options);
                header = { productMessage: { product: { ...product, productImage: imageMessage } } };
            }
            else if (image) {
                header = await (0, exports.prepareWAMessageMedia)({ image }, options);
            }
            else if (video) {
                const videoContent = await (0, exports.generateWAMessageContent)({ video }, options);
                if (!videoContent.videoMessage) {
                    throw new boom_1.Boom('Carousel video header upload did not produce a videoMessage', { statusCode: 400 });
                }
                header = { videoMessage: videoContent.videoMessage };
            }
            const hasMediaAttachment = hasValidCarouselHeader(header);
            if (!hasMediaAttachment) {
                throw new boom_1.Boom('Carousel card requires a valid image, video, or product header', { statusCode: 400 });
            }
            const nativeFlowMessage = Types_1.WAProto.Message.InteractiveMessage.NativeFlowMessage.fromObject({ buttons: normalizeCarouselButtons(buttons || []) });
            return Types_1.WAProto.Message.InteractiveMessage.fromObject({
                header: { title: title || '', hasMediaAttachment, ...header },
                body: { text: body || '' },

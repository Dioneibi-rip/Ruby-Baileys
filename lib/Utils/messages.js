...
const generateWAMessageContent = async (message, options) => {
    ...
    else if ('cards' in message && Array.isArray(message.cards) && message.cards.length) {
        const interactiveMessage = {
            carouselMessage: {
                cards: await Promise.all(message.cards.map(async (card) => {
                    let carouselHeader = {};
                    if ('product' in card && !!card.product) {
                        const { imageMessage } = await (0, exports.prepareWAMessageMedia)({ image: card.product.productImage }, options);
                        carouselHeader.productMessage = Types_1.WAProto.Message.ProductMessage.fromObject({
                            ...card,
                            product: { ...card.product, productImage: imageMessage }
                        });
                    }
                    else {
                    ...
                    if ('text' in card && !!card.text) {
                        carouselCard.body = { text: card.text };
                    }
                    else {
                        if ('caption' in card && !!card.caption) {
                            carouselCard.header = {
                                title: card.title || '',
                        ...
                        if ('thumbnail' in card && !!card.thumbnail) {
                            carouselCard.jpegThumbnail = card.thumbnail;
                        }
                        Object.assign(carouselCard.header || (carouselCard.header = {}), carouselHeader);
                    }

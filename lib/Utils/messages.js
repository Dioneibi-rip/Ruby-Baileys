...
const mapStickerPackSticker = async (sticker, index, options, logger) => {
    ...
    const fileName = sanitizeStickerPackFileName(`${String(index + 1).padStart(3, '0')}-${metadata.fileName || ''}`, index);
    return {
        sticker: {
...
const prepareWAStickerPackMessage = async (message, options) => {
    ...
    const packUpload = await uploadStickerPackPart(stickerPackArchive, 'stickerPack', options, logger);

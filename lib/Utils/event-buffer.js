...
const MAX_HISTORY_CACHE_KEYS = 500;
const MAX_BUFFERED_HISTORY_ITEMS = 500;
class BoundedSet extends Set {
    constructor(maxSize = MAX_HISTORY_CACHE_KEYS) {
        super();
    ...
    add(value) {
        ...
        while (this.size > this.maxSize) {
            const oldestKey = this.values().next().value;
...
const pruneRecordToLimit = (record, maxSize = MAX_BUFFERED_HISTORY_ITEMS) => {
...
const pruneBufferedHistory = (historySets) => {
    pruneRecordToLimit(historySets.chats);
    pruneRecordToLimit(historySets.contacts);
    pruneRecordToLimit(historySets.messages);
};
const releaseHistorySetReferences = (historySets) => {
    historySets.chats = {};
    historySets.messages = {};
    historySets.contacts = {};
    historySets.empty = true;
};
/**
...
const makeEventBuffer = (logger) => {
    ...
    const historyCache = new BoundedSet();
    ...
    function flush(force = false) {
        ...
        if (consolidatedData['messaging-history.set']) {
            releaseHistorySetReferences(data.historySets);
        }
        data = newData;
...
function append(data, historyCache, event, eventData, logger) {
    ...
    switch (event) {
        case 'messaging-history.set':
            ...
            pruneBufferedHistory(data.historySets);
            break;
        case 'chats.upsert':

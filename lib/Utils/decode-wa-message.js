...
const extractAddressingContext = (stanza) => {
    ...
    const addressingMode = stanza?.attrs?.addressing_mode || ((0, WABinary_1.isLidUser)(sender) ? 'lid' : 'pn');
    let senderAlt;
    let recipientAlt;
...
function decodeMessageNode(stanza, meId, meLid) {
    ...
    if ((0, WABinary_1.isJidGroup)(chatId) || (0, WABinary_1.isJidBroadcast)(chatId)) {
        key.participantAlt = addressingContext.senderAlt;
    }
    else {
        key.remoteJidAlt = addressingContext.senderAlt || addressingContext.recipientAlt;
    }
    const fullMessage = {
        key,

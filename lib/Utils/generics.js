...
exports.Browsers = {
    ...
    baileys: (browser) => ['Ubuntu', browser, '24.04.2'],
    windows: (browser) => ['Windows', browser, '11.0.26100'],
...
const extractWaWebVersion = (data) => {
    const candidates = [data?.currentVersion, data?.version, data?.waWebVersion, data?.webVersion, data?.latestVersion];
...
const fetchLatestWaWebVersion = async (options = {}) => {
    ...
    try {
        const response = await fetchWithTimeout('https://web.whatsapp.com/check-update?version=1&platform=web', { ...options, headers });
        const data = await response.json();
        const checkedVersion = extractWaWebVersion(data);
        if (checkedVersion) {
            return { version: checkedVersion, isLatest: true };
        }
    }
    catch (_) {
        // fall back to the service worker revision below
    }
    try {
        const response = await fetchWithTimeout('https://web.whatsapp.com/sw.js', { ...options, headers });

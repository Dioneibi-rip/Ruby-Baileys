...
/**
 * @deprecated Unsafe. A LID user part is an opaque Local ID, NOT a phone number,
 * so it cannot be converted to a PN JID by swapping the domain - doing so
 * fabricates a `@s.whatsapp.net` JID for a number that may not exist. Resolve
 * LID -> PN through the signal repository's stored mapping (or the `*Alt` JID
 * fields carried on incoming keys) instead.
 *
 * Retained for backwards compatibility. It now rebuilds the JID through
 * jidDecode/jidEncode so device and agent parts survive intact, and it no longer
 * touches hosted domains.
 */
const lidToJid = jid => {
    ...
    return jidEncode(decoded.user, 's.whatsapp.net', decoded.device)
}
